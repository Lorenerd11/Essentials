...
function init()
  ...
  self.liquidBoostTimer = 0
  self.liquidModifier = 1.5
  setFlightMode((mcontroller.liquidPercentage() > 0.5 and true or (world.gravity(mcontroller.position()) == 0)))
...
function update(dt)
  ...
  if mcontroller.liquidPercentage() >= 0.5 then
    self.liquidBoostTimer = 0
    self.submerged = true
  elseif self.submerged and mcontroller.liquidPercentage() < 0.5 and self.jumpBoostTimer == 0 and self.flightMode then
    self.submerged = false
    self.jumpBoostTimer = self.jumpBoostTime * (self.liquidModifier * 2)
    self.liquidBoostTimer = self.jumpBoostTime * self.liquidModifier
    mcontroller.setYVelocity(self.jumpVelocity * 2)
    animator.playSound("jump")
  end
  
  if self.liquidBoostTimer > 0 then
    mcontroller.setYVelocity(math.log(self.jumpVelocity, self.liquidModifier) / self.liquidModifier * self.liquidBoostTimer * (self.jumpVelocity * 0.2) + 0.5)
    self.liquidBoostTimer = math.max(0, self.liquidBoostTimer - dt)
  end

  setFlightMode(self.submerged or world.gravity(mcontroller.position()) == 0)
